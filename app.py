...
CURRENT_WEEK = "11주차"            # 해당 주차
ADMIN_PASSWORD = "3383"          # 선생님용 비밀번호

# 퀴즈 데이터 (수동으로 제공해주신 7문항 반영)
QUIZ_DATA = [
    {"q": "1. 현실적 여건을 고려하지 않은 재무적 열망(aspiration)과 현실적 여건을 고려한 재무적 기대(expectation) 사이의 격차는 재무적 (_________)이다.", "a": "아노미"},
    {"q": "2. 추상적인 개념 안에는 다양한 특성과 속성 또는 의미들이 포함되는데 이런 것들 중에서 다른 것과 구분되어 특정화될 수 있는 개념의 한 측면을 (_________)이라고 한다.", "a": "차원"},
    {"q": "3. 추상성이 높고 복잡한 개념은 여러 (_______)로 측정한다.", "a": "지표"},
    {"q": "4. 개념을 조작적으로 정의하고 그것을 측정하게 되면 그 결과는 고정된 값이 아니라 변화되는 다양한 값으로 나타나는데, 이런 상태의 개념을 (__________)라고 한다.", "a": "변수"},
    {"q": "5. 가설에서 원인에 의해 발생하는 결과에 해당하는 변수(결과변수)는 (___________)이다.", "a": "종속변수"},
    {"q": "6. 모든 상관관계는 인과관계일까? (예/아니요 중에서 맞는 것을 적으세요)", "a": "아니요"},
    {"q": "7. 측정의 수준에 따라 담고 있는 정보에 차이가 있고, 비율측정>등간측정>서열측정>(________)측정의 순으로 더 많은 정보를 제공한다.", "a": "명목"}
]
